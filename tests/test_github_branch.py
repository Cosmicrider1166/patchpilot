...
def build_repair_prompt(project, test_result):
    """Build the repair prompt for Claude."""
...
def validate_repair_path(file_path):
    """Prevent Claude from escaping the repository."""
...
async def main():
    ...
    try:
        ...
        if not final_result["passed"]:
            raise RuntimeError(
                "Repair did not make the tests pass."
            )

        print(
            "SUCCESS: Branch + repair workflow completed."
        )
